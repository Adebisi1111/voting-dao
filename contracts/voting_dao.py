...
class VotingDAO(gl.Contract):
    ...
    voters: TreeMap[str, VoterRecord]  # voter_address -> VoterRecord
    ...
    def register_voter(self) -> None:
        """Register to vote. Requires 0.1 GEN to prevent spam."""
        if gl.message.value < u256(100000000000000000):
            raise gl.vm.UserError("Registration requires 0.1 GEN")
    ...
    def resolve_proposal(self, proposal_id: str) -> str:
        ...
        CRITICAL: Validators must agree on the FINAL RESULT, not individual
        vote counts. This prevents validator-compatible counts from crossing
        thresholds and changing the outcome.
        """
        proposal = self.proposals.get(proposal_id, None)
        if proposal is None:
            raise gl.vm.UserError(f"Proposal {proposal_id} not found")
        ...
        # Validator: re-compute and compare FINAL RESULT
        def validator(leaders_res) -> bool:
            ...
            try:
                ...
                return False
            # CRITICAL: Agree only if the FINAL RESULT matches
