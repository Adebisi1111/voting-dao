...
STAKE_AMOUNT = 100000000000000000  # 0.1 GEN
...
class VotingDAO(gl.Contract):
    ...
    def stake(self) -> None:
        """Stake 0.1 GEN to become a voter."""
        sender = gl.message.sender_address.as_hex
        
        if gl.message.value != STAKE_AMOUNT:
            raise gl.vm.UserError(f"Must stake exactly 0.1 GEN ({STAKE_AMOUNT} wei)")
        
        rec = self.voters.get(sender, None)
        if rec is not None and rec.staked:
            raise gl.vm.UserError("Already staked")
        
        self.voters[sender] = Voter(staked=True, last_proposal_voted="", vote_count=u256(0))
        self.total_staked += u256(STAKE_AMOUNT)

    @gl.public.write
    def unstake(self) -> None:
        """Unstake GEN. Must not have any active votes."""
        sender = gl.message.sender_address.as_hex
        rec = self.voters.get(sender, None)
        
        if rec is None or not rec.staked:
            raise gl.vm.UserError("Not staked")
        
        if int(rec.vote_count) > 0:
            raise gl.vm.UserError("Cannot unstake with active votes. Wait for proposals to resolve.")
        ...
        gl.get_contract_at(Address(sender)).emit_transfer(value=u256(STAKE_AMOUNT))
    ...
    def cast_vote(self, proposal_id: str, vote: str) -> None:
        ...
        if rec is None or not rec.staked:
            raise gl.vm.UserError("Must stake 0.1 GEN to vote")
        ...
        def validator(leaders_res) -> bool:
            if not isinstance(leaders_res, gl.vm.Return):
                try:
                    ...
                    return str(getattr(e, "message", "")) == str(getattr(leaders_res, "message", ""))
                except Exception:
                    return False
            try:
                mine = leader_work()
    ...
    def is_voter(self, address: str) -> str:
        """Check if address has staked."""
        addr = address.lower() if hasattr(address, 'lower') else str(address).lower()
        rec = self.voters.get(addr, None)
        return "true" if rec is not None and rec.staked else "false"

    @gl.public.view
    def get_vote_count(self, address: str) -> str:
        """Get total votes cast by a user."""
        addr = address.lower() if hasattr(address, 'lower') else str(address).lower()
        rec = self.voters.get(addr, None)
        if rec is None:
            return "0"
        return str(int(rec.vote_count))

    @gl.public.view
    def get_stake_amount(self) -> str:
        """Get required stake amount."""
        return str(STAKE_AMOUNT)
